...
// Rota para enviar o arquivo task-details.html com o parâmetro de consulta id
server.get('/task-details', async (request, response) => {
    const tarefaId = request.query.id; // Obtém o ID da tarefa da query string

    if (!tarefaId) {
        return response.status(400).send({ error: 'ID da tarefa é obrigatório' });
    }

    return response.sendFile('task-details.html');
});
...
server.put('/tarefas/:id', async (request, response) => {
    ...
    try {
        ...
        return response.status(500).send({ message: "Erro ao atualizar a tarefa" });
    }
});
